<template>
  <br />
  <div style="height: fit-content; margin-top: 10px; position: relative">
    <header id="head">
      <h3 class="ml-5 text-h5" style="cursor: pointer">
        Sports<img style="width: 30px" src="" />
      </h3>
      <v-spacer></v-spacer>
      <div
        class="progress"
    ...
    >
      <swiper-slide
        id="swip"
        v-for="pro in this.catigoryProducts3"
        :key="pro.id"
      >
        <v-card id="card">
          <v-row>
...
export default {
  ...
  async mounted() {
    await this.getCatigoryProduct3("sports-accessories");
  ...
  methods: {
    ...
    ...mapActions(mystore, ["getCatigoryProduct3"]),
    async lovefun(data) {
  ...
  computed: {
    ...mapState(mystore, ["catigoryProducts3"]),
