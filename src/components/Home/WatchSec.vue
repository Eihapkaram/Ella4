<template>
  ...
  <div id="conn">
    ...
    >
      <swiper-slide
        id="swip"
        v-for="pro in this.catigoryProducts5"
        :key="pro.id"
      >
        <v-card id="card">
          <v-row>
...
export default {
  ...
  async mounted() {
    await this.getCatigoryProduct5("mens-watches");
  ...
  methods: {
    ...
    ...mapActions(mystore, ["getCatigoryProduct5"]),
    async lovefun(data) {
  ...
  computed: {
    ...mapState(mystore, ["catigoryProducts5"]),
